...
These files are needed to enable state outputs (or enhanced state outputs with statistics/counters) in specific MAME roms/games via LUA scripts, which benefit those with force feedback.
...
Please refer to the "Progress Notes" to understand what is being worked on and what to look out for prior to the final release. As things stand right now, it ahould only be a few games requiring official Game Status memory address identification, but in the meantime fallback logic has been used which prevents recoil and rumble during attract mode on initial game bootup, but after playing a game, dying, and returning to the main menu, then recoil and rumble may operate during attract mode (only initial attract mode upon bootup is ignored with fallback logic).

Special thanks to Muggins for all of his help in beta testing with me and verifying compatibility with Sinden light guns. And a special thanks to the original developer (unknown as people advised me they dont want to be contacted) who created the original MAME LUA script, which prompted me to create this version from the ground up for easy upgrades and collaboration across the community.
