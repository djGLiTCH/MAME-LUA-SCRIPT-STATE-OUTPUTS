...
# output names and writes output/<channel>/stateoutput/native_outputs_by_rom.lua (plus the scrape-report
...
# script falls back to the MAME_SRC_PATH constant near the top of msop_native_outputs_compiler.py -
...
#   --no-scrape-report      skip the audit JSON (native_outputs_by_rom.lua is always written)
set -e
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PY="$(command -v python3 || command -v python)"
"$PY" "$DIR/msop_native_outputs_compiler.py" "$@"
echo
