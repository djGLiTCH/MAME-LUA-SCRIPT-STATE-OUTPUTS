...
REM   2. driver compiler      (MAME source                ->  output\beta\stateoutput\database_driver.lua) [optional]
...
if "%MAME_SRC%"=="" (
    ...
    python "%~dp0msop_database_driver_compiler.py" --channel beta --mame-src "%MAME_SRC%"
    if errorlevel 1 goto :error
)
