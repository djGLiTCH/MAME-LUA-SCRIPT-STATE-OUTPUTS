...
REM   4. MAMEhooker .ini       (stable database  ->  output\stable\ini, --exclude-driver)  [MSOP outputs only]
...
if exist "%~dp0..\output\stable\stateoutput\native_outputs_by_rom.lua" (
    echo === [STABLE / MSOP-only] removing native_outputs_by_rom.lua from the plugin folder ===
    del /q "%~dp0..\output\stable\stateoutput\native_outputs_by_rom.lua"
)
if exist "%~dp0..\output\stable\stateoutput\native_outputs_by_driver.lua" (
...
python "%~dp0msop_mamehooker_ini_generator.py" --channel stable --exclude-driver
if errorlevel 1 goto :error
