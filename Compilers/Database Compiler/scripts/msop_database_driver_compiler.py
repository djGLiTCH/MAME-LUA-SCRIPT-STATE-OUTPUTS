...
#
# Every run writes database_driver.lua directly: every ROM found (via games/
...
def channel_output_dir(channel):  return os.path.join(BASE_DIR, "output", channel, "stateoutput")   # database_driver.lua
...
# debugging/auditing aid; database_driver.lua does not depend on it. Set this to
# False to skip creating that file entirely. --scrape-report / --no-scrape-report
# on the command line override this per run. (database_driver.lua is always
...
def get_supported_roms(games_dir):
    """Every *.json in games/ except _default.json and any _-prefixed template."""
    roms = []
...
def print_summary(mame_src, report, report_path, out_path):
    ...
    print("-" * 70)
    print(f" database_driver.lua      : {out_path}")
    if report_path:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        "MSOP-supported ROM and (re)compile database_driver.lua on every run. "
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Directory to write database_driver.lua into "
    ...
    parser.add_argument(
        ...
        "database_driver.lua is always written either way.",
    )
    ...
    # Always (re)generate database_driver.lua - it's this script's primary output and is
    ...
    out_path = write_database_driver_lua(output_dir, driver_data)
