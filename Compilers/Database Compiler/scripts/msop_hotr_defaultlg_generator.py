...
#
# Output: output/<channel>/defaultLG/<rom>.txt  (CRLF, NO trailing newline - matches the hand-made
# examples). --channel stable|beta selects both the input database and this output tree.
...
from msop_output_model import (  # noqa: E402
    BASE_DIR, REPO_ROOT, resolve_players, supported_roms, load_default, load_game,
    write_lines, diff_summary, set_channel,
)

REF_DIR = os.path.join(REPO_ROOT, "Hook Of The Reaper", "defaultLG")  # shipped examples, for --report
EOL = "\r\n"  # HOTR reads CRLF; the hand-made examples have NO trailing newline.
...
def main():
    ...
    ap.add_argument("--report", action="store_true", help="diff every generated file against the shipped examples")
    ...
    for rom in supported_roms():
        if args.rom and rom != args.rom:
            continue
        text = write_lines(os.path.join(out_dir, rom + ".txt"), hotr_lines(load_game(rom), default), EOL, trailing=False)
        written += 1
        if args.report:
            d = diff_summary(text, os.path.join(REF_DIR, rom + ".txt"))
            exact += (d is None)
    ...
    print("-" * 78)
    if args.report:
        print(f"  {exact}/{written} exact vs examples (differences are the intended mapping-policy corrections)")
