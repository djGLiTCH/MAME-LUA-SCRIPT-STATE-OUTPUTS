...
REM output names and writes output\<channel>\stateoutput\native_outputs_by_rom.lua (plus the scrape-report
...
REM script falls back to the MAME_SRC_PATH constant near the top of msop_native_outputs_compiler.py -
...
REM   --no-scrape-report      skip the audit JSON (native_outputs_by_rom.lua is always written)

python "%~dp0msop_native_outputs_compiler.py" %*
if errorlevel 1 (
    echo.
...
echo.
echo Done - native_outputs_by_rom.lua written under output\^<channel^>\stateoutput\.
pause
exit /b 0
