...
--
-- This plugin subscribes to TWO separate, deliberately different hooks per machine
-- session - do not merge them or change which one does what:
--
--   1. emu.register_devices_started -> Register_Master_List()
--      Fires exactly once, after devices exist but BEFORE MAME locks its output
--      table for save states. This is the ONLY point where root:register_output()
--      is safe to call. Requires a patched MAME core (see luaengine.cpp/machine.cpp
--      in this project's companion patch) - falls back to emu.register_prestart on
--      unpatched builds, which still works for live play but registers outputs too
--      late for them to be captured in save states.
--
...
--
-- Getting these two swapped, merged, or reordered is the single easiest way to
-- silently break output registration - it will not throw an error, it will just
-- mean root:register_output() calls have nowhere safe left to run before
-- MAME's output table locks, and every output write for the rest of the
-- session falls back to the deprecated global set_value() path.
-- =========================================================================================
...
function stateoutput.startplugin()
    ...
    -- Dynamically scrapes the database and registers the Master List of
    -- synthetic output names against the root device.
    -- -------------------------------------------------------------------------
    local function Register_Master_List()
        ...
        -- 1. Scrape every ROM entry's OUTPUT_SUFFIXES, including "_default".
        -- "_default" alone already guarantees the core global outputs
        -- (Credits, GameStatus, AttractStatus, LuaVersion, LuaDate, etc.)
        -- register correctly-named even before a specific ROM is known -
        -- there used to also be a hand-maintained `base_list` of hardcoded
        -- fallback names here, but it had drifted out of sync with the
        -- actual suffix strings below (e.g. it registered "MSOP_Game_Status"
        -- while every write in Frame_Logic targets "MSOP_GameStatus") and
        -- was redundant with this scrape whenever the database loads
        -- successfully. If the database ever fails to load, on_start()
        -- already disables every ROM outright, so a hardcoded fallback
        -- output list would never have received a value anyway - removed
        -- rather than re-synced, so this naming can't drift again.
        ...
        -- 2. Custom outputs synthesized directly in Resolve_Addresses_And_Strings
        -- (DemulShooter compatibility names) - these are NOT part of any
        -- ROM's OUTPUT_SUFFIXES table, so the scrape above can never see
        -- them; they have to be listed explicitly here.
        for i = 1, 4 do
            unique_outputs["MSOP_P"..i.."_CtmRecoil"] = true
            unique_outputs["MSOP_P"..i.."_Damaged"] = true
        end

        -- 3. Register everything, tracking REAL success/failure per name.
        -- Now every failure is logged individually with its actual error,
        -- and the summary line reports genuine successes out of the true total.
        ...
        for name, _ in pairs(unique_outputs) do
            ...
            elseif ok then
                failures[#failures + 1] = name .. ": register_output() returned false - see MAME log for the matching [LUA ERROR] line"
            else
                failures[#failures + 1] = name .. ": " .. tostring(result)
            end
        end
        dbg_print("Register_Master_List Complete. Registered " .. success_count .. "/" .. total .. " unique state outputs.")
        if #failures > 0 then
    ...
    if emu.register_devices_started then
        dbg_print("Hooked Register_Master_List to devices_started (pre-lock)")
        emu.register_devices_started(Register_Master_List)
    ...
    -- 3. STOP Phase: Cleanup and save-state restoration
