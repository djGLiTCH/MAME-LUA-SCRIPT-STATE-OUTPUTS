...
#
# Every run writes database_driver.lua directly: every ROM found (via game_json/
...
def main():
    ...
    parser.add_argument(
        "--out",
        default="mame_driver_native_output_scrape_report.json",
        help="Path for the scrape-report JSON (only used when the report is enabled)",
    )
    parser.add_argument(
        "--scrape-report",
