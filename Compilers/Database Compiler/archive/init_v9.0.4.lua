...
function stateoutput.startplugin()
    ...
    -- Can MAME create new outputs on this build (root.register_output present)? Set for real
    -- by Register_Master_List, which always runs before on_start - devices_started, prestart
    -- and start all precede machine_reset. Declared HERE rather than beside its only other
    -- user because resolve_relay_mode reads it, and a local declared later in the file is not
    -- in scope for a function compiled earlier.
    --
    -- On its own this says nothing about DELIVERY: register_output is absent on every stock
    -- build, 0.288 and 0.289 alike. It only matters combined with the set_value probe - see
    -- the capability gate in resolve_relay_mode.
    ...
    -- KEEP IN SYNC with the header + exports.version above (904 == v9.0.4).
    ...
    local function resolve_relay_mode()
        ...
        -- CAPABILITY GATE (v9.0.4). Ground truth, and it outranks both inferences below.
        -- Ask this build whether it can hold a custom output at all - by BOTH routes:
        --   * register_output (PR 15639 forks only), and
        --   * the deprecated output.set_value(), probed for real by Probe_Legacy_Set_Value.
        -- If neither works, MAME cannot store our outputs, so it cannot broadcast them in ANY
        -- -output mode, and the relay is the only delivery path there is.
        --
        ...
        --
        -- NOTE the probe, not _RegisterOutputSupported alone: register_output is absent on
        -- EVERY stock build, 0.288 included, so that flag alone would drag a genuine old build
        -- onto a relay it does not need. set_value's create behaviour is the version-dependent
        -- one, and only the two together mean "structurally impossible".
        --
        -- Only ever forces the relay ON, never off.
        if _LegacyCreateProbed and not _LegacyCreateWorks and not _RegisterOutputSupported then
            _UseRelay = true
            dbg_print("Relay mode: ENABLED (this build can create no custom outputs by either "
                      .. "route - MAME has nothing of ours to broadcast in any -output mode)")
            return
        end
    ...
    -- (declared far earlier - see the engine-state block near the top - because
    -- resolve_relay_mode needs it and Lua only binds a name to a local that is already
    -- in scope where the reference is compiled.)
