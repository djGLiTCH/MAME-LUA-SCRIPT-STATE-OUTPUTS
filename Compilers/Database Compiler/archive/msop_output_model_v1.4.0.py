...
def set_channel(channel):
    ...
    global CHANNEL, DB_GAMES, DRIVER_LUA, _GAME_FILES
    CHANNEL    = channel
    DB_GAMES   = os.path.join(BASE_DIR, "input", channel, "database", "games")
    DRIVER_LUA = os.path.join(BASE_DIR, "output", channel, "stateoutput", "native_outputs_by_rom.lua")
    _GAME_FILES = None
...
# rom -> absolute path, discovered RECURSIVELY: game files may live directly in games/
# or in any subfolder (games/lightgun/, games/racing/, ...). Cached per channel (cleared
# by set_channel). First occurrence wins on a duplicate stem - the database compiler
# treats duplicates as a build error, so a clean tree never has one.
_GAME_FILES = None

def _game_files():
    global _GAME_FILES
    if _GAME_FILES is None:
        files = {}
        for dirpath, dirnames, filenames in os.walk(DB_GAMES):
            dirnames.sort()
            for name in sorted(filenames):
                if name.endswith(".json") and not name.startswith("_"):
                    files.setdefault(name[:-5], os.path.join(dirpath, name))
        _GAME_FILES = files
    return _GAME_FILES


def load_game(rom):
    return load_json(_game_files().get(rom) or os.path.join(DB_GAMES, rom + ".json"))


def supported_roms():
    for rom in sorted(_game_files()):
        yield rom
