...
function stateoutput.startplugin()
    ...
    local PLUGIN_DATE_NUM    = 20260705
    ...
    local function broadcast_native_event(event_string)
        ...
        if connected and sock then
            pcall(function() sock:write(event_string) end)
        end
    end
    ...
    local function msop_out(out_name, val)
        if val ~= last_state[out_name] then
            payload_buffer = payload_buffer .. string.format("%s = %d\r\n", out_name, val)
    ...
    --     connect attempt.
    --   - The first frame after any (re)connect is silently absorbed rather
    --     than sent: last_state was just wiped by connect_socket(), so
    --     without this, every already-nonzero output would look like a
    --     fresh change and flood the relay with a "nothing really happened"
    --     burst the moment the socket opens.
    --   - A failed or zero-byte write tears down the whole connection
    --     (rather than just skipping this frame's payload), because a
    --     half-closed TCP socket doesn't reliably surface as an error until
    --     the next syscall - treating any write failure as "the connection
    --     is gone" and forcing a full reconnect is simpler and safer than
    --     trying to distinguish a transient failure from a dead peer.
    local function flush_payload_to_relay()
        ...
        if changes_detected then
            ...
            local success, bytes_written = pcall(function() return sock:write(payload_buffer) end)
    ...
    local function Set_Output_Safe(out_handle, name, value)
        ...
        -- stock builds). Harmless no-op otherwise: Get_Output_Proxy's cached
        -- proxy will be false for a name that was never created, and the
        -- legacy fallback resolves to the same read-only lookup under the
        -- hood - so this can't succeed where the relay write above wasn't
        -- already covering it. Restores save-state persistence and
        -- on_post_load's adopt() recovery wherever creation did succeed.
    ...
    local function Compute_Outputs()
        -- Ensure the ROM is valid and memory spaces are bound
        if not CFG then return end
