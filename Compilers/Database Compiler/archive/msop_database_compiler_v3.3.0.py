...
# Compiler Version: 3.3.0
# Compiler Date: 2026.07.12
...
#   python msop_database_compiler.py stable   -> plugin.json channel = "stable", then mode 1
#   python msop_database_compiler.py beta     -> plugin.json channel = "beta",   then mode 1
# The channel lands in input/stateoutput/plugin.json (and therefore in the
# assembled output/stateoutput/ copy), where the Configurator reads it to
# display which channel an installed plugin came from. run_stable.bat/.sh and
# run_beta.bat/.sh in this folder wrap these two calls.
#
...
SOURCE_DIR  = os.path.join(BASE_DIR, "input", "stateoutput")   # init.lua/plugin.json/readme.txt
# The game-data source: database.json plus the per-game games/ folder (two interchangeable formats).
DATABASE_DIR = os.path.join(BASE_DIR, "input", "database")
...
BUILD_DIR   = os.path.join(BASE_DIR, "output", "stateoutput")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ("stable", "beta"):
            print_banner()
            print(f" HEADLESS: release channel -> {arg} (mode 1, no prompts)")
    ...
    print("       INPUT:  input/database/games/*.json")
    print("       OUTPUT: input/database/database.json  +  output/stateoutput/database.lua\n")
    print(" [ 2 ] Compile from the single Database JSON")
    print("       INPUT:  input/database/database.json")
    print("       OUTPUT: input/database/games/*.json  +  output/stateoutput/database.lua")
    print("-" * 70)
