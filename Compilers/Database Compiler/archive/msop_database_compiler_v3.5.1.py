...
def format_lua_value(val, indent=1):
    """Recursively serialises a Python value into Lua table literal syntax
...
def sync_project_metadata(master_db, channel_override=None):
    """Synchronises plugin version/date across the project, treating
    ...
    if final_ver_str != "0.0.0" and os.path.exists(plugin_path):
        try:
            ...
            # The "relay" and "relayport" members are DEPLOY-TIME stamps the MESH app
            # patches into each installed copy (per MAME instance). They must never exist
            # in the source or the shipped zip - a manual install has to self-detect and
            # use the default port - so strip them if they ever leak into the input copy
            # (e.g. someone copied plugin.json back from a live MAME install). Output/ is
            # assembled from this file, so this also guarantees the shipped copy stays
            # pristine.
            for stamp in ("relay", "relayport"):
                if stamp in plugin_data.get("plugin", {}):
                    del plugin_data["plugin"][stamp]
                    updated = True
...
def compile_from_folder(channel_override=None):
    ...
    # If Mode 1 dynamically updated the _default block, save it back to the source folder to keep it synchronised
    ...
    # A parse error in any game file (or in _default.json) is a hard failure: the compiled database is
    # incomplete, so return non-zero. This lets the run_*/run launchers stop at THIS step (with the
    # [ERROR] file/line above) instead of running on into a downstream generator crash.
    if stats["error"] > 0 or default_status == "Parse Error":
        print(f"\n [BUILD INCOMPLETE] {stats['error']} game file(s) failed to parse - see the [ERROR] line(s)")
        print( "                    above for the file and line number. The compiled database EXCLUDES them.")
        return 1
    return 0
...
def split_from_master():
    ...
    if stats["error"] > 0 or default_status == "Write Error":
        print(f"\n [BUILD INCOMPLETE] {stats['error']} game file(s) could not be written - see the [ERROR] line(s) above.")
        return 1
    return 0
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ("stable", "beta"):
            ...
            sys.exit(compile_from_folder(channel_override=arg) or 0)
        print_banner()
    ...
    if choice == '1':
        sys.exit(compile_from_folder() or 0)
    elif choice == '2':
        sys.exit(split_from_master() or 0)
    else:
