...
**Alternative:** Open Command Prompt (`cmd`), use the `cd` command to navigate to the MSOP Compiler folder, type msop_database_compiler_windows.bat, and press Enter.
...
4. Make the script executable by running this command (you only need to do this if you are using MSOP Compiler for the first time):

    **`chmod +x msop_database_compiler_linux_macos.sh`**
6. Execute the script by running this command:

    **`./msop_database_compiler_linux_macos.sh`**
8. A black Command Prompt window will appear, displaying "Script Started..."
9. In the black Command Prompt window, a menu will appear with two options:
   * **`[ 1 ] Compile from folder with individual game JSONs`**
   * **`[ 2 ] Compile from single Database JSON`**
10. Type **`1`** or **`2`** as appropriate, and hit Enter


#### How The MSOP Compiler Works

Depending on whether you selected option 1 or 2, the MSOP Compiler script will:
1. Read all individual `.json` files in your `game_json` folder or the combined `.json` database file
2. Validate all games for correct json parsing syntax (and if there are any errors, you will be notified of the specific line that has the issue)
3. Modify the opposing individual `.json` game profiles or single `database.json` file to keep everything in sync
4. Generate a new `database.lua` with all of the new game profiles

Your new MSOP Database configuration (`database.lua`) is now ready to be used in MAME with the MSOP Plugin.
...
* **`MEMORY_SPACE`**: Tells MAME which emulated CPU contains the game's core logic (Defaults: `"program"`).
